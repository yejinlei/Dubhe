...
export const parseDicom = (byteArray, filename) => {
  try {
    ...
    return {rows, columns, patientID, seriesInstanceUID, studyInstanceUID, modality, bodyPartExamined, filename, SOPInstanceUID};
  } catch(err) {
...
export const readDicoms = async (files) => {
  const mapper = (file) => new Promise((resolve, reject) => {
    const reader = new FileReader();
