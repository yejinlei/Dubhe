  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <cdOperation>
        <span slot="right">
          <!-- 搜索 -->
          <el-input v-model="query.blurry" clearable placeholder="输入菜单名称或路由地址搜索" style="width: 230px;" class="filter-item" @change="crud.toQuery" />
          <el-date-picker
    ...
    >
      <el-form ref="form" :inline="true" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="菜单类型" prop="type">
          <el-radio-group v-model="form.type" class="long-item" :disabled="isEdit" @change="onChangeType">
        ...
        <el-form-item label="菜单标题" prop="name">
          <el-input v-model="form.name" class="short-item" placeholder="菜单标题" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item label="菜单图标" prop="icon">
          <el-autocomplete
            ...
            @select="handleSelect"
          >
            <IconFont v-if="form.icon" slot="prefix" :type="form.icon" :style="{ marginLeft: '5px' }" />
        ...
        <el-form-item label="上级菜单" prop="pid">
          <treeselect v-model="form.pid" :defaultExpandLevel="1" :clearable="false" :searchable="false" :options="menus" class="short-item" placeholder="选择上级类目" />
        </el-form-item>
        <el-form-item label="菜单排序" prop="sort">
          <el-input-number v-model.number="form.sort" :min="0" :max="999" />
        </el-form-item>
        <el-form-item v-if="!isLink" label="路由地址" prop="path">
          <el-input v-model="form.path" placeholder="相对上级菜单的路由地址" class="short-item" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item v-if="isLink" label="链接地址" prop="path">
          <el-input v-model="form.path" placeholder="以http(s)://开头" class="long-item" maxlength="100" show-word-limit />
        </el-form-item>
        <el-form-item v-if="!isLink" label="权限标识" prop="permission">
          <el-input v-model="form.permission" placeholder="权限标识" class="short-item" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item v-if="isPage" label="路由名称" prop="componentName">
          <el-input v-model="form.componentName" class="short-item" placeholder="路由名称" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item v-if="isPage" label="组件路径" prop="component">
          <el-input v-model="form.component" class="short-item" placeholder="相对/src/views/的路径" maxlength="50" show-word-limit />
        </el-form-item>
        <el-form-item v-if="isPage" label="页面布局" prop="layout">
          <el-select v-model="form.layout" placeholder="页面布局" class="short-item" clearable>
            <el-option v-for="item in dict.Layout" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column label="操作" width="120" fixed="right">
        <template slot-scope="scope">
          <udOperation :data="scope.row" msg="确定删除吗,如果存在下级节点则一并删除，此操作不能撤销！" />
        </template>
      </el-table-column>
    </el-table>
  </div>
...
import { validateName, validateString } from '@/utils/validate';
...
// crud交由presenter持有
const defaultForm = { id: null, name: null, sort: 999, path: null, component: null, componentName: null, layout: null, roles: [], pid: 0, icon: null, cache: false, hidden: false, type: 0, permission: null };
export default {
  name: 'Menu',
  components: { BaseModal, Treeselect, cdOperation, rrOperation, udOperation },
  cruds() {
    return CRUD({ title: '菜单', crudMethod: { ...crudMenu }});
  },
