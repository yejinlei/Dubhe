...
<template>
  <div class="layout-root layout-same-style" style="width: 100%;">
    <el-dialog title="图片保存选项" :visible.sync="dialogFormVisible" class="layout-svg-save-dialog">
      <el-form :model="form">
    ...
    <el-dialog title="全力加载中,请稍等..." :visible.sync="initWaitVisible" class="layout-svg-save-dialog">
    ...
    <el-dialog title="设定数据同步间隔" :visible.sync="dataSyncVisible" class="layout-svg-save-dialog">
      <div>
        <el-checkbox v-model="syncDataBool">启用同步</el-checkbox>
      </div>
      <div style="margin-top: 10px;">
        <el-input-number v-model="timeSync" class="input-number" size="mini" :min="30" :disabled="!syncDataBool" />秒
      </div>
    </el-dialog>
    <div ref="left" class="layout-sidebar layout-same-style" :style="{height: leftStyle.height, width: leftStyle.width}">
      <logo :collapse="false" />
          ...
          >
            ...
            <div ref="eachMiddle" :class="['layout-sidebar-category-each-middle']" :style="item.id==selected||i==initId && initFlag?{borderLeft:'4px solid #2e4fde'}:{}">
              ...
              >
                ...
                <span :class="[leftStyle.categoryFlage?'span-after':'']" :style="item.id==selected||i==initId && initFlag?{fontSize: 16 + 'px' , color: 'rgb(46, 79, 222)' }:{fontSize: 16 + 'px'}">{{ item.name }}</span>
              </router-link>
            </div>
    ...
    <div class="layout-header-content layout-same-style" :style="{height: rightStyle.height, width: rightStyle.width}">
      <div class="layout-header layout-same-style">
        ...
        <div :class="['same-div', 'search-tag']" style="width: 400px;">
          ...
          >
            <i :class="['el-icon-refresh']" style="font-size: 19px; color: whitesmoke; cursor: pointer;" @click="isClicked" />
            <i
...
const { mapMutations: mapLayoutMutations, mapActions: mapLayoutActions, mapGetters: mapLayoutGetters, mapState: mapLayoutState } = createNamespacedHelpers('Visual/layout');
const {
    ...
    if (typeof (params.id) === 'undefined' || typeof (params.trainJobName) === 'undefined') {
      ...
      if (typeof (Storage) !== 'undefined') {
        if (sessionStorage.id && sessionStorage.trainJobName) {
          ...
          this.setErrorMessage(`${'没有参数 id 和 trainJobName!!!' +
          '_'}${  new Date().getTime()}`);
        }
      } else {
        this.setErrorMessage(`${'对不起，你的浏览器不支持 web storage!!!' +
        '_'}${  new Date().getTime()}`);
      ...
      if (val === 'download') {
        if (this.downloadState.indexOf(this.selectedRaw) >= 0) {
          if (this.svgDownloadList[this.selectedRaw].length === 0) {
            this.setErrorMessage(`${'请先在当前页面勾选需要下载的图片' +
          '_'}${  new Date().getTime()}`);
          } else {
            this.dialogFormVisible = true;
          }
        } else {
          this.setErrorMessage(`${'当前页面不支持下载' +
          '_'}${  new Date().getTime()}`);
        ...
        const {className} = document.getElementsByClassName('category-selected')[0].getElementsByTagName('i')[0];
...
<style lang="less" scoped>
    ...
    .layout-svg {
      * {
      ...
      .in-text-selection,
      ::selection { color: var(--select-text-font-color); text-shadow: none; background: var(--select-text-bg-color); }
      #write { position: relative; width: inherit; height: auto; padding-top: 40px; margin: 0 auto; overflow-x: visible; word-break: normal; word-wrap: break-word; white-space: normal; }
      #write.first-line-indent p { text-indent: 2em; }
      #write.first-line-indent li p,
      #write.first-line-indent p * { text-indent: 0; }
      #write.first-line-indent li { margin-left: 2em; }
      .for-image #write { padding-right: 8px; padding-left: 8px; }
      body.typora-export { padding-right: 30px; padding-left: 30px; }
      .typora-export .footnote-line,
      .typora-export li,
      .typora-export p { white-space: pre-wrap; }

      @media screen and (max-width: 500px) {
        body.typora-export { padding-right: 0; padding-left: 0; }
        #write { padding-right: 20px; padding-left: 20px; }
        .CodeMirror-sizer { margin-left: 0 !important; }
        .CodeMirror-gutters { display: none !important; }
      }
      #write li > figure:last-child { margin-bottom: 0.5rem; }
      #write ol,
      #write ul { position: relative; }
      img { max-width: 100%; vertical-align: middle; image-orientation: from-image; }
      button,
      input,
      select,
      textarea { font-family: inherit; font-size: inherit; font-style: inherit; font-weight: inherit; font-stretch: inherit; line-height: inherit; color: inherit; font-variant-caps: inherit; }
      input[type="checkbox"],
      input[type="radio"] { padding: 0; line-height: normal; }
      *,
      ::after,
      ...
      h4,
      h5,
      h6 { break-after: avoid-page; break-inside: avoid; orphans: 4; }
      p { orphans: 4; }
      h1 { font-size: 2rem; }
      h2 { font-size: 1.8rem; }
      h3 { font-size: 1.6rem; }
      ...
      .hidden { display: none; }
      .md-blockmeta { font-style: italic; font-weight: 700; color: rgb(204, 204, 204); }
      a { cursor: pointer; }
      sup.md-footnote { padding: 2px 4px; color: rgb(85, 85, 85); cursor: pointer; background-color: rgba(238, 238, 238, 0.7); border-top-left-radius: 4px; border-top-right-radius: 4px; border-bottom-right-radius: 4px; border-bottom-left-radius: 4px; }
      sup.md-footnote a,
      sup.md-footnote a:hover { color: inherit; text-decoration: inherit; text-transform: inherit; }
      #write input[type="checkbox"] { width: inherit; height: inherit; cursor: pointer; }
      figure { max-width: calc(100% + 16px); padding: 0; margin: 1.2em 0; overflow-x: auto; }
      figure > table { margin: 0; }
      tr { break-inside: avoid; break-after: auto; }
      thead { display: table-header-group; }
      table { width: 100%; overflow: auto; text-align: left; border-spacing: 0; border-collapse: collapse; break-inside: auto; }
      table.md-table td { min-width: 32px; }
      .CodeMirror-gutters { background-color: inherit; border-right-width: 0; }
      ...
      .md-fences { position: relative !important; display: block; overflow: visible; font-size: 0.9rem; text-align: left; white-space: pre; background-color: inherit; background-image: inherit; background-repeat: inherit inherit; background-attachment: inherit; background-position: inherit inherit; background-clip: inherit; background-origin: inherit; background-size: inherit; break-inside: avoid; }
      .md-diagram-panel { width: 100%; padding-top: 0; padding-bottom: 8px; margin-top: 10px; overflow-x: auto; text-align: center; }
      #write .md-fences.mock-cm { white-space: pre-wrap; }
      .md-fences.md-fences-with-lineno { padding-left: 0; }
      #write.fences-no-line-wrapping .md-fences.mock-cm { overflow-x: auto; white-space: pre; }
      .md-fences.mock-cm.md-fences-with-lineno { padding-left: 8px; }
      .CodeMirror-line,
      twitterwidget { break-inside: avoid; }
      .footnotes { margin-top: 1em; margin-bottom: 1em; font-size: 0.9rem; opacity: 0.8; }
      .footnotes + .footnotes { margin-top: 0; }
      .md-reset { position: static; box-sizing: content-box; float: none; width: auto; height: auto; padding: 0; margin: 0; font-weight: 400; line-height: normal; text-align: left; text-decoration: none; text-shadow: none; white-space: nowrap; vertical-align: top; cursor: inherit; background-repeat: initial initial; background-position: 0 0; border: 0; outline: 0; direction: ltr; }
      ...
      .footnotes-area { padding-bottom: 0.143rem; margin-top: 0.714rem; color: rgb(136, 136, 136); white-space: normal; }
      #write .footnote-line { white-space: pre-wrap; }

      @media print {
        body,
        html { height: 99%; border: 1px solid transparent; break-after: avoid; break-before: avoid; font-variant-ligatures: no-common-ligatures; }
        #write { padding-top: 0; margin-top: 0; border-color: transparent !important; }
        .typora-export * { -webkit-print-color-adjust: exact; }
        html.blink-to-pdf { font-size: 13px; }
        .typora-export #write { padding-right: 32px; padding-bottom: 0; padding-left: 32px; break-after: avoid; }
        .typora-export #write::after { height: 0; }
        .is-mac table { break-inside: avoid; }
      }
      .footnote-line { margin-top: 0.714em; font-size: 0.7em; }
      a img,
      img a { cursor: pointer; }
      pre.md-meta-block { display: block; min-height: 0.8rem; overflow-x: hidden; font-size: 0.8rem; white-space: pre-wrap; background-color: rgb(204, 204, 204); background-repeat: initial initial; background-position: initial initial; }
      p > .md-image:only-child:not(.md-img-error) img,
      p > img:only-child { display: block; margin: auto; }
      #write.first-line-indent p > .md-image:only-child:not(.md-img-error) img { position: relative; left: -2em; }
      ...
      [contenteditable="false"]:focus { outline: 0; box-shadow: none; }
      .md-task-list-item { position: relative; list-style-type: none; }
      .task-list-item.md-task-list-item { padding-left: 0; }
      .md-task-list-item > input { position: absolute; top: 0; left: 0; margin-top: calc(1em - 10px); margin-left: -1.2em; border: none; }
      .math { font-size: 1rem; }
      .md-toc { position: relative; min-height: 3.58rem; font-size: 0.9rem; border-top-left-radius: 10px; border-top-right-radius: 10px; border-bottom-right-radius: 10px; border-bottom-left-radius: 10px; }
      .md-toc-content { position: relative; margin-left: 0; }
      .md-toc-content::after,
      .md-toc::after { display: none; }
      ...
      pre,
      samp,
      tt { font-family: var(--monospace); }
      kbd { padding: 0.1em 0.6em; margin: 0 0.1em; font-size: 0.8em; color: rgb(36, 39, 41); white-space: nowrap; vertical-align: middle; background-color: rgb(255, 255, 255); background-repeat: initial initial; background-position: initial initial; border: 1px solid rgb(173, 179, 185); border-top-left-radius: 3px; border-top-right-radius: 3px; border-bottom-right-radius: 3px; border-bottom-left-radius: 3px; box-shadow: rgba(12, 13, 14, 0.2) 0 1px 0, rgb(255, 255, 255) 0 0 0 2px inset; }
      .md-comment { font-family: var(--monospace); color: rgb(162, 127, 3); opacity: 0.8; }
      code { text-align: left; }
      a.md-print-anchor { position: absolute !important; right: 0 !important; display: inline-block !important; width: 1px !important; text-shadow: initial !important; white-space: pre !important; background-repeat: initial initial !important; background-position: 0 0 !important; border: none !important; outline: 0 !important; }
      .md-inline-math .MathJax_SVG .noError { display: none !important; }
      .html-for-mac .inline-math-svg .MathJax_SVG { vertical-align: 0.2px; }
      .md-math-block .MathJax_SVG_Display { position: relative; display: block !important; width: auto; min-width: 100%; max-width: none; min-height: 0; max-height: none; margin: 0; overflow-y: hidden; text-align: center; text-indent: 0; }
      .MathJax_SVG_Display,
      .md-inline-math .MathJax_SVG_Display { display: inline-block !important; width: auto; margin: inherit; }
      .MathJax_SVG .MJX-monospace { font-family: var(--monospace); }
      .MathJax_SVG .MJX-sans-serif { font-family: sans-serif; }
      .MathJax_SVG { display: inline; float: none; min-width: 0; max-width: none; min-height: 0; max-height: none; padding: 0; margin: 0; font-style: normal; font-weight: 400; line-height: normal; text-align: left; text-indent: 0; text-transform: none; letter-spacing: normal; word-wrap: normal; white-space: nowrap; zoom: 90%; border: 0; word-spacing: normal; direction: ltr; }
      .MathJax_SVG * { transition: none; }
      .MathJax_SVG_Display svg { margin-top: 0 !important; margin-bottom: 0 !important; vertical-align: middle !important; }
      .os-windows.monocolor-emoji .md-emoji { font-family: "Segoe UI Symbol", sans-serif; }
      .md-diagram-panel > svg { max-width: 100%; }
      [lang="flow"] svg,
      [lang="mermaid"] svg { max-width: 100%; height: auto; }
      ...
      .highlight td,
      .highlight tr { border: 0; }
      svg[id^="mermaidChart"] { line-height: 1em; }
      mark { color: rgb(0, 0, 0); background-color: rgb(255, 255, 0); background-repeat: initial initial; background-position: initial initial; }
      ...
      .typora-export p,
      .typora-export,
      .footnote-line { white-space: normal; }
