...
export default {
  ...
  setup(props, ctx) {
    ...
    const afterUpload = () => {
      ...
      if (state.errFileList.length && state.errFileList.length + uploadedFiles === state.fileList.length) {
        // 当有错误文件且文件总数匹配时
    ...
    const onClickConfirm = () => {
      ...
      switch (state.activeStep) {
        case 0:
          uploadedFiles = 0;
          state.errFileList = [];
          upload.value.uploadSubmit((resolved, total) => {
            if (localUploadId !== uploadId) { return; }
            uploadedFiles = resolved;
            const percent = toFixed(resolved / total);
