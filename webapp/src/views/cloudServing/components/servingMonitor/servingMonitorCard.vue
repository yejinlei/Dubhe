...
<template>
  <el-card class="serving-monitor-card">
    ...
    >
      <div
        class="info-wrapper"
        :class="{cp: pod.grafanaUrl}"
        @click="onMonitorClick"
      >
        <div
          v-if="modelUsages.length"
          class="monitor-units-container"
        >
...
<script>
import { cpuFormatter, cpuPercentage, memFormatter, memPercentage } from '@/views/cloudServing/util';
...
export default {
  ...
  computed: {
    modelUsages() {
      ...
      list.push({
        ...
        percentage: cpuPercentage(cpuUsageAmount, cpuUsageFormat, cpuRequestAmount, cpuRequestFormat),
      });
      ...
      list.push({
        ...
        percentage: memPercentage(memoryUsageAmount, memoryUsageFormat, memoryRequestAmount, memoryRequestFormat),
      });

      if (gpuUsed > 0) {
        const gpuPercentageTotal = gpuUsagePersent.reduce((total, gpu) => total + Number(gpu.usage), 0);
        list.push({
          title: 'GPU',
  ...
  methods: {
    onMonitorClick() {
      if (this.pod.grafanaUrl) {
        const newPage = window.open();
        newPage.location = this.pod.grafanaUrl;
      }
    },
  },
};
