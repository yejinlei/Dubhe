...
def getdirsize(dir):
    ...
    for root, dirs, files in os.walk(dir):
        subdir = root.split('/')[-1]
        for name in files:
            if (not ((subdir.endswith('-m') or subdir.endswith('-v')))) and \
                    (('teacher' not in subdir) or ('student' in subdir) \
                    or ('layer' not in subdir)) and ('bert' in subdir):
                sum_size += os.path.getsize(os.path.join(root, name))
    return sum_size
...
def run_eval_job(eval_job_func, num_steps, desc='train'):
    ...
    model_size = getdirsize(args.model_load_dir)
