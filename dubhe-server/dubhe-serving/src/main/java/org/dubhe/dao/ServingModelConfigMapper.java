...
public interface ServingModelConfigMapper extends BaseMapper<ServingModelConfig> {
     ...
     * @return
     */
    @Select("select * from serving_model_config where serving_id = #{servingId} and deleted = 1 order by create_time desc")
     ...
     * @return
     */
    @Select("select * from serving_model_config where serving_id = #{servingId} and deleted = 0")
    List<ServingModelConfig> selectByServingId(@Param("servingId") Long servingId);
