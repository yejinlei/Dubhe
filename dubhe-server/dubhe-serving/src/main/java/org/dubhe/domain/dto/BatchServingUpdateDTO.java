...
public class BatchServingUpdateDTO implements Serializable {
    private static final long serialVersionUID = 1L;

    @ApiModelProperty(value = "批量服务id")
    @NotNull(message = "批量服务ID不能为空")
    private Long id;

    @ApiModelProperty(value = "批量服务名称", required = true)
    @NotNull(message = "批量服务名称不能为空")
    @Size(max = 50, message = "批量服务名称长度超过50")
    @Pattern(regexp = StringConstant.REGEXP, message = "批量服务名称仅支持字母、数字、汉字、英文横杠和下划线")
    private String name;

    @ApiModelProperty(value = "批量服务类型：0-Restful，1-gRPC", required =true)
    @Min(value = NumberConstant.NUMBER_0, message = "批量服务类型参数错误")
    @Max(value = NumberConstant.NUMBER_1, message = "批量服务类型参数错误")
    @TableField("type")
    ...
    @ApiModelProperty(value = "模型路径", required =true)
