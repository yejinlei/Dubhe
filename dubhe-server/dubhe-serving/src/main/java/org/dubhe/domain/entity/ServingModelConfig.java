...
@TableName(value = "serving_model_config")
public class ServingModelConfig extends BaseEntity {
     ...
     * 部署参数
     */
    @TableField(value = "deploy_params")
    ...
    public String getDeployParam() {
        JSONObject deployParam = JSONObject.parseObject(deployParams.replace("\\", ""));
        StringBuilder params = new StringBuilder();
        for (String key : deployParam.keySet()) {
            if(Objects.nonNull(deployParam.get(key))){
                params.append(" --").append(key).append("='").append(deployParam.get(key)).append("'");
            }
        }
