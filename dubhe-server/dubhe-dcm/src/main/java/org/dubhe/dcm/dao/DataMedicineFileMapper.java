...
public interface DataMedicineFileMapper extends BaseMapper<DataMedicineFile> {
     ...
     */
    @Select("select url from data_medicine_file where medicine_id = #{medicineId}")
     ...
     * 更新文件状态
     *
     * @param medicine_id  文件ID
     * @param status       文件状态
     */
    @Update("update data_medicine_file set status = #{status} where medicine_id = #{medicine_id}")
    void updateStatus(@Param("medicine_id") Long medicine_id, @Param("status") Integer status);

    /**
     * 更新文件状态
     *
     * @param medicalId 数据集ID
     * @param status    状态
     */
    @Update("update data_medicine_file set status = #{status} where medicine_id=#{medicalId}")
    void batchUpdateStatus(@Param("medicalId")Long medicalId, @Param("status") Integer status);
     ...
     * @return 文件状态列表
     */
    @Select("select status from data_medicine_file where medicine_id=#{medicineId} group by status")
    List<Integer> getFileStatusUnion(@Param("medicineId")Long medicineId);
     ...
     */
    @Select("<script>"
            + " select * from data_medicine_file where id in "
            + " <foreach collection='fileIds' item='id' open='(' separator=',' close=')'> "
            + " #{id}"
            + " </foreach>"
            + "</script>")
    ...
    void deleteMedicineFile(@Param("id") Long id);
