...
public class MedicineAnnotationServiceImpl implements MedicineAnnotationService {
    ...
    public boolean finishAuto() {
        ...
        if (ObjectUtil.isNotNull(object)) {
            String detailKey = object.toString();
            Object taskDetailObject = redisUtils.get(detailKey);
            JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(taskDetailObject));
            Long taskId = jsonObject.getLong("taskId");
            JSONArray dcmsArray = jsonObject.getJSONArray("dcms");
            ...
            List<Long> medicineFileIds = JSON.parseObject(jsonObject.getString("medicineFileIds"), ArrayList.class);
            DcmStateMachineUtil.stateChange(new StateChangeDTO() {{
