...
@RefreshScope
@Component(value = "tadlAlgorithmRecycleFile")
public class TadlAlgorithmRecycleFile extends AbstractGlobalRecycle {
    @Autowired
    ...
    protected void rollback(RecycleCreateDTO dto) {
        if (Objects.isNull(dto)||Objects.isNull(dto.getRemark())){
            LogUtil.error(LogEnum.TADL,"算法文件恢复异常");
            return;
        }
        //恢复算法版本
        algorithmVersionService.updateAlgorithmVersionStatus(Long.valueOf(dto.getRemark()),Boolean.FALSE);
        algorithmStageService.updateStageStatusByVersionId(Long.valueOf(dto.getRemark()),Boolean.FALSE);
    }
}
