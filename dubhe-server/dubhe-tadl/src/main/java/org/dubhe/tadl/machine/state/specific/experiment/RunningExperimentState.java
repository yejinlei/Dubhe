...
@Component
public class RunningExperimentState extends AbstractExperimentState {
    @Autowired
    @Lazy
    ...
    public void pausedExperimentEvent(Long experimentId) {
        ...
        experimentService.updateExperiment(new LambdaUpdateWrapper<Experiment>()
                .eq(Experiment::getId,experimentId)
                        .set(Experiment::getStatus,ExperimentStatusEnum.PAUSED_EXPERIMENT_STATE.getValue()));
        experimentStateMachine.setMemoryExperimentState(experimentStateMachine.getPausedExperimentState());
    ...
    public void finishedExperimentEvent(Long experimentId) {
        ...
        experimentService.updateExperiment(new LambdaUpdateWrapper<Experiment>()
                .eq(Experiment::getId,experimentId)
                        .set(Experiment::getStatus,ExperimentStatusEnum.FINISHED_EXPERIMENT_STATE.getValue())
                        .set(Experiment::getEndTime,new Timestamp(System.currentTimeMillis()))
        );
        experimentStateMachine.setMemoryExperimentState(experimentStateMachine.getFinishedExperimentState());
    ...
    public String currentStatus() {
        return ExperimentStatusEnum.RUNNING_EXPERIMENT_STATE.getMsg();
    }
}