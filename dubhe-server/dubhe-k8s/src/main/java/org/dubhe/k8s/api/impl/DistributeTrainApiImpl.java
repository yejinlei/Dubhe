...
import static org.dubhe.k8s.constant.K8sParamConstants.*;
...
public class DistributeTrainApiImpl implements DistributeTrainApi {
    ...
    public PtBaseResult deleteByResourceName(String namespace, String resourceName) {
        ...
        try {
            ...
            //判断该分布式训练资源是否存在
            if(items.size() == 0) {
                return new PtBaseResult(K8sResponseEnum.NOT_FOUND.getCode(), K8sResponseEnum.NOT_FOUND.getMessage());
            }
            //判断是否存在多个分布式训练资源
            if(items.size() > 1) {
    ...
    public BizDistributeTrain findDisByName(String crName) {
        ...
        if(!CollectionUtil.isEmpty(distributeTrains)){
            for (DistributeTrain distributeTrain : distributeTrains) {
                ...
                if(metadata!=null){
                    if(crName.equals(metadata.getName())){
                        return BizConvertUtils.toBizDistributeTrain(distributeTrain);
                    }
                }
            }
    ...
    public BizDistributeTrain create(String crYaml) {
        try {
            ...
            LogUtil.error(LogEnum.BIZ_K8S, "Create DistributeTrain error:{} ,yml:{}", e,crYaml);
            return null;
        }
    }
    /**
    ...
    public boolean delete(String crYaml) {
        try {
            ...
            LogUtil.error(LogEnum.BIZ_K8S, "Delete DistributeTrain error:{} ,yml:{}", e,crYaml);
            return false;
        }
    }
