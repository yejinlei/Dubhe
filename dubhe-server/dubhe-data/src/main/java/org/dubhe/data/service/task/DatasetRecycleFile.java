...
public class DatasetRecycleFile extends AbstractGlobalRecycle {
    ...
    public void clear(Object object) {
        LogUtil.info(LogEnum.BIZ_DATASET,"Class DatasetRecycleFile execute clear method , param:{}", JSONObject.toJSONString(object));
        ...
        if(!Objects.isNull(recycleTask.getRecycleCondition())){
            ...
            //校验超时时间  && 延迟一秒 循环批量删除
            try {
                while (checkoutOverTime(recycleTask) && fileMapper.deleteBydatasetId(datasetId,LIMIT_NUMBER ) > 0) {
                    TimeUnit.SECONDS.sleep(1);
                }
            }catch (Exception e){
                LogUtil.info(LogEnum.BIZ_DATASET,"Class DatasetRecycleFile execute clear method error , param:{} error:{}",
