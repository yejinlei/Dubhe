 ...
 */
@Component
public class NotAnnotationDcmFileState extends AbstractDcmFileState {
    ...
    public void annotationEvent(List<Long> fileIds) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件前内存中状态机的状态 :{} ", dcmFileStateMachine.getMemoryFileState());
        ...
        dcmFileStateMachine.setMemoryFileState(dcmFileStateMachine.getAutoAnnotationCompleteDcmFileState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件后内存状态机的切换： {}", dcmFileStateMachine.getMemoryFileState());
    }
    /**
    ...
    public void autoAnnotationSaveEvent(List<Long> fileIds) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件前内存中状态机的状态 :{} ", dcmFileStateMachine.getMemoryFileState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 接受参数： {} ", fileIds);
        dataMedicineFileMapper.updateStatusByIds(fileIds, DcmFileStateEnum.AUTO_ANNOTATION_COMPLETE_FILE_STATE.getCode());
        dcmFileStateMachine.setMemoryFileState(dcmFileStateMachine.getAutoAnnotationCompleteDcmFileState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【未标注】 执行事件后内存状态机的切换： {}", dcmFileStateMachine.getMemoryFileState());
    }
