 ...
 */
@Service
public class AnnotationDataState extends AbstractDataMedicineState {
    ...
    public void autoAnnotationEvent(Long primaryKeyId) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注中】 执行事件前内存中状态机的状态 :{} ", dcmDataMedicineStateMachine.getMemoryDataMedicineState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 接受参数： {} ", primaryKeyId);
        dataMedicineMapper.updateStatus(primaryKeyId, DcmDataStateEnum.AUTOMATIC_LABELING_STATE.getCode());
    ...
    public void annotationCompleteEvent(DataMedicine medical) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注中】 执行事件前内存中状态机的状态 :{} ", dcmDataMedicineStateMachine.getMemoryDataMedicineState());
        LogUtil.debug(LogEnum.STATE_MACHINE, " 接受参数： {} ", medical);
        dataMedicineMapper.updateStatus(medical.getId(), DcmDataStateEnum.ANNOTATION_COMPLETE_STATE.getCode());
