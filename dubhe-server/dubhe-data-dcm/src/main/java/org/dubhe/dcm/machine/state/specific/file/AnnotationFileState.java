 ...
 */
@Service
public class AnnotationFileState extends AbstractDcmFileState {
    ...
    /**
     * 文件 标注中-->自动标注文件-->自动标注完成
     *
     * @param fileIds 文件ID
     */
    @Override
    public void autoAnnotationEvent(List<Long> fileIds) {
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注中】 执行事件前内存中状态机的状态 :{} ", dcmFileStateMachine.getMemoryFileState());
        ...
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注中】 执行事件后内存状态机的切换： {}", dcmFileStateMachine.getMemoryFileState());
    }
