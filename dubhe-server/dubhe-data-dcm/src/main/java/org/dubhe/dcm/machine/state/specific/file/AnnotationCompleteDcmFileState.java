 ...
 */
@Component
public class AnnotationCompleteDcmFileState extends AbstractDcmFileState {
    ...
    public void annotationSaveEvent(List<Long> fileIds){
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注完成】 执行事件前内存中状态机的状态 :{} ", dcmFileStateMachine.getMemoryFileState());
        ...
        LogUtil.debug(LogEnum.STATE_MACHINE, " 【标注完成】 执行事件后内存状态机的切换： {}", dcmFileStateMachine.getMemoryFileState());
    }
